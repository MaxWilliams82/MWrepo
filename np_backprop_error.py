...
while 1:
  ...
  d_biasm = 0
  d_biasn = 0
  d_wo = np.array([0,0,0,0])
  d_wk = np.array([0,0,0,0])
  d_wl = np.array([0,0,0,0])
  d_wm = np.array([0,0,0,0])
  d_wn = np.array([0,0,0,0])
  ...
  for j in range(data_ml.nb_data):
    ...
    d_wk += (zo - target)*deriveesigmoid(yo)*wo[0]*deriveesigmoid(yk)*x
    d_wl += (zo - target)*deriveesigmoid(yo)*wo[1]*deriveesigmoid(yl)*x
    d_wm += (zo - target)*deriveesigmoid(yo)*wo[2]*deriveesigmoid(ym)*x
    d_wn += (zo - target)*deriveesigmoid(yo)*wo[3]*deriveesigmoid(yn)*x
